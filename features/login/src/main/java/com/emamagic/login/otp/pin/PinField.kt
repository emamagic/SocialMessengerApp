...
open class PinField : AppCompatEditText {
    ...
    protected var tmpHighlightPaint = Paint()

    protected var yPadding = Util.dpToPx(10f)

    protected var tmpHighlightSingleFieldType = HighlightType.ALL_FIELDS
    ...
    fun invalidInput() {
        ...
        highlightPaint.color = ContextCompat.getColor(context, com.emamagic.common_ui.R.color.limoo_error_state)
        highlightSingleFieldType = HighlightType.ALL_FIELDS
        handler.postDelayed({reset()}, 1500)
    }

    private fun reset() {
        highlightSingleFieldType = tmpHighlightSingleFieldType
        highlightPaint.color = tmpHighlightPaint.color
        setText("")
    }
...
//        }
    ...
    override fun onTextChanged(
        ...
        if (text != null && text.length == numberOfFields - 1) {
            onTextCompleteListener?.onTextNotComplete()
        }
    }
